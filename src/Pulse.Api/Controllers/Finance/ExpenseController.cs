...
using MoneyMovement = Pulse.Core.Models.MoneyMovement;
    ...
    public async Task<ActionResult> Update(int id, DailyExpenseCreateDto dto)
    {
        ...
        try
        {
            await strategy.ExecuteAsync(async () =>
            {
                ...
                var existingMovement = await _db.MoneyMovements
                    .FirstOrDefaultAsync(m => m.RelatedExpenseId == id && m.IsAutoGenerated && m.UserId == UserId);
                    ...
                    dto.FundingSourceId.HasValue && dto.ToFundingSourceId.HasValue)
                {
                    ...
                    {
                        existingMovement.SourceType = MoneyMovementEntityType.BankAccount;
                        existingMovement.SourceId = dto.FundingSourceId.Value;
                        existingMovement.DestinationType = MoneyMovementEntityType.BankAccount;
                        existingMovement.DestinationId = dto.ToFundingSourceId.Value;
                        existingMovement.Amount = dto.Amount;
                        existingMovement.MovementDate = expense.Date;
                        existingMovement.MovementType = MovementType.Transfer;
                        existingMovement.Note = dto.Description;
                    }
                    else
                    {
                        _db.MoneyMovements.Add(new MoneyMovement
                    ...
                    dto.FundingSourceId.HasValue && dto.ToFundingSourceId.HasValue)
                {
                    if (existingMovement != null)
                    {
                        existingMovement.SourceType = MoneyMovementEntityType.BankAccount;
                        existingMovement.SourceId = dto.FundingSourceId.Value;
                        existingMovement.DestinationType = MoneyMovementEntityType.CreditCard;
                        existingMovement.DestinationId = dto.ToFundingSourceId.Value;
                        existingMovement.Amount = dto.Amount;
                        existingMovement.MovementDate = expense.Date;
                        existingMovement.MovementType = MovementType.CardPayment;
                        existingMovement.Note = dto.Description;
                    }
                    else
                    {
                ...
                {
                    _db.MoneyMovements.Remove(existingMovement);
                    await _db.SaveChangesAsync();
                }
    ...
    public async Task<ActionResult> Delete(int id)
    {
        ...
        try
        {
            await strategy.ExecuteAsync(async () =>
            {
                ...
                var relatedMovement = await _db.MoneyMovements
                    .FirstOrDefaultAsync(m => m.RelatedExpenseId == id && m.IsAutoGenerated && m.UserId == UserId);
                if (relatedMovement != null)
                    _db.MoneyMovements.Remove(relatedMovement);
