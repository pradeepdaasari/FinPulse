...
public class RecurringController : ControllerBase
{
    ...
    public async Task<ActionResult> Pay(int id, [FromBody] DailyExpenseCreateDto dto)
    {
        ...
        try
        {
            await strategy.ExecuteAsync(async () =>
            {
                ...
                if (dto.TransactionType == TransactionType.Transfer && fundingSourceId.HasValue && dto.ToFundingSourceId.HasValue)
                {
                    ...
                    if (source != null) { await _db.Entry(source).ReloadAsync(); source.CurrentBalance -= dto.Amount; }
                    if (dest != null) { await _db.Entry(dest).ReloadAsync(); dest.CurrentBalance += dto.Amount; }
                }
        ...
        {
            return StatusCode(500, new { error = "Failed to record recurring payment. Please try again." });
        }
    }
